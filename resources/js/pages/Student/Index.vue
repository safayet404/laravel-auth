...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="p-4">
            ...
            <EasyDataTable :headers="headers" :items="students" :rows-per-page="10" buttons-pagination
                table-class-name="customize-table" class="rounded-lg shadow-md">
                <template #item-permissions="{ permissions }">
                    <div class="flex flex-wrap gap-1">
                        <span v-for="p in permissions" :key="p.id"
                ...
                <template #item-operation="item">
                    <div class="flex items-center gap-2 py-2">
                        <Link :href="`/roles/${item.id}`"
                            class="rounded bg-blue-600 px-3 py-1 text-xs text-white hover:bg-blue-700">
                            Show
                        </Link>

                        <Link v-if="can('roles.edit')" :href="`/roles/${item.id}/edit`"
                            class="rounded bg-gray-200 px-3 py-1 text-xs text-black hover:bg-gray-300 dark:bg-white">
                            Edit
                        </Link>

                        <button v-if="can('roles.delete')" @click="deleteRole(item.id)"
